...
export default function FamilyListPage() {
    ...
    if (!searchQuery.trim()) {
      ...
      const filtered = members.filter(
        (member) =>
          ...
          member.domicile?.toLowerCase().includes(query)
      );
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      const treeId = '2'; // TODO: Make this dynamic

      const response = await axios.get<APIResponse<{ members: FamilyMember[] }>>(
        API_ENDPOINTS.familyTree(treeId),
        {
          headers: {
      ...
      if (response.data.code === 200 && response.data.data) {
        ...
        setFilteredMembers(response.data.data.members || []);
      }
    } catch (err) {
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
        <div className="relative">
          ...
          {searchQuery && (
            ...
            >
              <svg className="h-5 w-5 text-zinc-400 hover:text-zinc-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-zinc-600 dark:text-zinc-400">
              {searchQuery ? 'No members found matching your search' : 'No family members yet'}
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="divide-y divide-zinc-200 dark:divide-zinc-700">
              {filteredMembers.map((member) => (
                <div
                  key={member.id}
                  className="px-6 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors"
                >
                  {/* Desktop View */}
                  <div className="hidden md:grid md:grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-2">
                      ...
                      >
                        <Image 
                          src={member.gender === 'male' ? '/icons/male.svg' : '/icons/female.svg'} 
                          width={12} 
                          height={12} 
                          alt="gender"
                        />
                    ...
                    <div className="col-span-2 flex justify-end gap-2">
                      <button
                        onClick={() => router.push(`/admin/family/edit/${member.id}`)}
                  ...
                  <div className="md:hidden space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <span className={`ml-2 font-medium ${member.gender === 'male' ? 'text-blue-600 dark:text-blue-400' : 'text-pink-600 dark:text-pink-400'}`}>
                          {member.gender === 'male' ? 'Pria' : 'Wanita'}
                        </span>
                      </div>
                      <div>
